...
export class bseefourthSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('4th-year-bsee-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      getFourthBseeSchedules: this.coeService.getFourthBseeSchedules(),
    }).subscribe({
      next: ({ getFourthBseeSchedules }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFourthBseeSchedules];
