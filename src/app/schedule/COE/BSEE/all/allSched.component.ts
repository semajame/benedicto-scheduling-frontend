...
export class bseeallSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubjects: this.coeService.findMinorSubjectsBsee(),
      technoSubject: this.ccsService.findTechnoForCoe(),
      allSchedules: this.coeService.getAllBseeSchedule(),
    }).subscribe({
            ...
            this.teacherService.getTeacherByName(teacher).subscribe({
              next: (teacherData) => {
                if (teacherData && teacherData.employee_id) {
                  ...
                  this.coeService.addBseeSchedule(newAppointment).subscribe({
                    next: (response) => {
      ...
      next: (subjectData) => {
        ...
        this.teacherService.getTeacherByName(teacher).subscribe({
          next: (teacherData) => {
            ...
            this.coeService
              .updateBseeSchedule(appointment.id, updatedAppointment)
              .subscribe({
                next: (response) => {
  ...
  AppointmentDelete(event: any): void {
    ...
    if (confirm('Are you sure you want to delete this appointment?')) {
      this.coeService.deleteBseeSchedule(appointment.id).subscribe({
        next: () => {
