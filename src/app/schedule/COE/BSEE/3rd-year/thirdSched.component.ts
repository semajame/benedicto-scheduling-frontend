...
export class bseethirdSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      ...
      getThirdBseeSchedules: this.coeService.getThirdBseeSchedules(),
    }).subscribe({
      next: ({ getThirdBseeSchedules, minorSubjects }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getThirdBseeSchedules, ...minorSubjects];
