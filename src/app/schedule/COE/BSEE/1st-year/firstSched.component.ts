...
export class bseefirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bsee-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubjects: this.coeService.findMinorSubjectsBseeFirstYear(),
      getFirstBseeSchedules: this.coeService.getFirstBseeSchedules(),
    }).subscribe({
      next: ({ getFirstBseeSchedules, minorSubjects }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBseeSchedules, ...minorSubjects];
