...
export class bsmesecondSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('2nd-year-bsme-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsmeSecondYear(),
      techoSubject: this.cssService.findTechnoForCoe(),
      getSecondBsmeSchedules: this.coeService.getSecondBsmeSchedules(),
    }).subscribe({
      next: ({ techoSubject, getSecondBsmeSchedules, minorSubject }) => {
