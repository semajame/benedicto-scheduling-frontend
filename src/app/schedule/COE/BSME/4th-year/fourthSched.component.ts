...
export class bsmefourthSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('4th-year-bsme-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      getFourthBsmeSchedules: this.coeService.getFourthBsmeSchedules(),
    }).subscribe({
      next: ({ getFourthBsmeSchedules }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFourthBsmeSchedules];
