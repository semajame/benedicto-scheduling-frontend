...
export class bsmethirdSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('3rd-year-bsme-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsmeThirdYear(),

      getThirdBsmeSchedules: this.coeService.getThirdBsmeSchedules(),
    }).subscribe({
      next: ({ getThirdBsmeSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getThirdBsmeSchedules, ...minorSubject];
