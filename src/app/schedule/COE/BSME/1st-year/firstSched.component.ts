...
export class bsmefirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bsme-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsmeFirstYear(),
      getFirstBsmeSchedules: this.coeService.getFirstBsmeSchedules(),
    }).subscribe({
      next: ({ getFirstBsmeSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBsmeSchedules, ...minorSubject];
