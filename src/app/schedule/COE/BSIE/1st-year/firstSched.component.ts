...
export class bsiefirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bsie-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsieFirstYear(),

      getFirstBsieSchedules: this.coeService.getFirstBsieSchedules(),
    }).subscribe({
      next: ({ getFirstBsieSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBsieSchedules, ...minorSubject];
