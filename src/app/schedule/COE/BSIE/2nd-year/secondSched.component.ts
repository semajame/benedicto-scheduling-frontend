...
export class bsiesecondSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('2nd-year-bsie-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsieSecondYear(),
      getSecondBsieSchedules: this.coeService.getSecondBsieSchedules(),
    }).subscribe({
      next: ({ getSecondBsieSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getSecondBsieSchedules, ...minorSubject];
