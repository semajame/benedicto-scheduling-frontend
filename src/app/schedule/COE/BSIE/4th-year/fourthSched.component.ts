...
export class bsiefourthSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('4th-year-bsie-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      getFourthBsieSchedules: this.coeService.getFourthBsieSchedules(),
    }).subscribe({
      next: ({ getFourthBsieSchedules }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFourthBsieSchedules];
