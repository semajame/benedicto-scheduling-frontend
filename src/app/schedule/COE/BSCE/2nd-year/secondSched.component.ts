...
export class bscesecondSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('2nd-year-bsce-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.coeService.findMinorSubjectsBsceSecondYear(),
      techoSubject: this.cssService.findTechnoForCoe(),
      getSecondBsceSchedules: this.coeService.getSecondBsceSchedules(),
    }).subscribe({
      next: ({ techoSubject, getSecondBsceSchedules, minorSubject }) => {
