...
export class bscefirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bsce-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubjects: this.coeService.findMinorSubjectsBsceFirstYear(),

      getFirstBsceSchedules: this.coeService.getFirstBsceSchedules(),
    }).subscribe({
      next: ({ getFirstBsceSchedules, minorSubjects }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBsceSchedules, ...minorSubjects];
