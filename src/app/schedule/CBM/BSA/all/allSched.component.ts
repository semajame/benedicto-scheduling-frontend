...
export class bsaallSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsa(),
      getAllBsaSchedules: this.cbmService.getAllBsaSchedule(),
    }).subscribe({
      next: ({ getAllBsaSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getAllBsaSchedules, ...minorSubject];
            ...
            this.teacherService.getTeacherByName(teacher).subscribe({
              next: (teacherData) => {
                if (teacherData && teacherData.employee_id) {
                  ...
                  this.cbmService.addBsaSchedule(newAppointment).subscribe({
                    next: (response) => {
      ...
      next: (subjectData) => {
        ...
        this.teacherService.getTeacherByName(teacher).subscribe({
          next: (teacherData) => {
            ...
            this.cbmService
              .updateBsaSchedule(appointment.id, updatedAppointment)
              .subscribe({
                next: (response) => {
  ...
  AppointmentDelete(event: any): void {
    ...
    if (confirm('Are you sure you want to delete this appointment?')) {
      this.cbmService.deleteBsaSchedule(appointment.id).subscribe({
        next: () => {
