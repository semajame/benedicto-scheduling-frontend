...
export class bsafourthSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('4th-year-bsa-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsaFourthYear(),
      getFourthBsaSchedules: this.cbmService.getFourthBsaSchedules(),
    }).subscribe({
      next: ({ getFourthBsaSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFourthBsaSchedules, ...minorSubject];
