...
export class bsathirdSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('3rd-year-bsa-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsaThirdYear(),
      getThirdBsaSchedules: this.cbmService.getThirdBsaSchedules(),
    }).subscribe({
      next: ({ getThirdBsaSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getThirdBsaSchedules, ...minorSubject];
