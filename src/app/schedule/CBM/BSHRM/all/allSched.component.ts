...
export class bshrmallSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshrm(),
      getAllBshrmSchedules: this.cbmService.getAllBshrmSchedule(),
    }).subscribe({
      next: ({ getAllBshrmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getAllBshrmSchedules, ...minorSubject];
            ...
            this.teacherService.getTeacherByName(teacher).subscribe({
              next: (teacherData) => {
                if (teacherData && teacherData.employee_id) {
                  ...
                  this.cbmService.addBshrmSchedule(newAppointment).subscribe({
                    next: (response) => {
      ...
      next: (subjectData) => {
        ...
        this.teacherService.getTeacherByName(teacher).subscribe({
          next: (teacherData) => {
            ...
            this.cbmService
              .updateBshrmSchedule(appointment.id, updatedAppointment)
              .subscribe({
                next: (response) => {
  ...
  AppointmentDelete(event: any): void {
    ...
    if (confirm('Are you sure you want to delete this appointment?')) {
      this.cbmService.deleteBshrmSchedule(appointment.id).subscribe({
        next: () => {
