...
export class bshrmsecondSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('2nd-year-bshm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshrmSecondYear(),
      getSecondBshrmSchedules: this.cbmService.getSecondBshrmSchedules(),
    }).subscribe({
      next: ({ getSecondBshrmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getSecondBshrmSchedules, ...minorSubject];
