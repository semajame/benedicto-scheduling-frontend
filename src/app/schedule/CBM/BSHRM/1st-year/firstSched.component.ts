...
export class bshrmfirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bshrm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshrmFirstYear(),
      getFirstBshrmSchedules: this.cbmService.getFirstBshrmSchedules(),
    }).subscribe({
      next: ({ getFirstBshrmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBshrmSchedules, ...minorSubject];
