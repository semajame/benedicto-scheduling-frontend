...
export class bsmmallSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubjects: this.cbmService.findMinorSubjectsBsmm(),
      allSchedules: this.cbmService.getAllBsmmSchedule(),
    }).subscribe({
      next: ({ allSchedules, minorSubjects }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...allSchedules, ...minorSubjects];
                      ...
                      this.teacherService.getTeacherByName(teacher).subscribe({
                        next: (teacherData) => {
                          if (teacherData && teacherData.employee_id) {
                            ...
                            this.cbmService
                              .addBsmmSchedule(newAppointment)
                              .subscribe({
                                next: (response) => {
