...
export class bsmmsecondSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('2nd-year-bsmm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsmmSecondYear(),
      getSecondBsmmSchedules: this.cbmService.getSecondBsmmSchedules(),
    }).subscribe({
      next: ({ getSecondBsmmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getSecondBsmmSchedules, ...minorSubject];
