...
export class bsmmfirstSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('1st-year-bsmm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsmmFirstYear(),
      getFirstBsmmSchedules: this.cbmService.getFirstBsmmSchedules(),
    }).subscribe({
      next: ({ getFirstBsmmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFirstBsmmSchedules, ...minorSubject];
