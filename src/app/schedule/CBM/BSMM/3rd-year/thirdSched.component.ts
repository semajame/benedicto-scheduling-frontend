...
export class bsmmthirdSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('3rd-year-bsmm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBsmmThirdYear(),
      getThirdBsmmSchedules: this.cbmService.getThirdBsmmSchedules(),
    }).subscribe({
      next: ({ getThirdBsmmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getThirdBsmmSchedules, ...minorSubject];
