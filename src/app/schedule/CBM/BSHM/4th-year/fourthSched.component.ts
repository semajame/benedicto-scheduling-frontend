...
export class bshmfourthSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('4th-year-bshm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshmFourthYear(),
      getFourthBshmSchedules: this.cbmService.getFourthBshmSchedules(),
    }).subscribe({
      next: ({ getFourthBshmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getFourthBshmSchedules, ...minorSubject];
