...
export class bshmsecondSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshmSecondYear(),
      getSecondBshmSchedules: this.cbmService.getSecondBshmSchedules(),
    }).subscribe({
      next: ({ getSecondBshmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getSecondBshmSchedules, ...minorSubject];
