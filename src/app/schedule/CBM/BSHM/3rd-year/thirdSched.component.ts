...
export class bshmthirdSchedComponent implements AfterViewInit {
  ...
  generatePDF() {
    html2canvas(this.pdfContent.nativeElement, { scale: 2 }).then((canvas) => {
      ...
      pdf.save('3rd-year-bshm-schedule.pdf');
    });
  }
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubject: this.cbmService.findMinorSubjectsBshmThirdYear(),
      getThirdBshmSchedules: this.cbmService.getThirdBshmSchedules(),
    }).subscribe({
      next: ({ getThirdBshmSchedules, minorSubject }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...getThirdBshmSchedules, ...minorSubject];
