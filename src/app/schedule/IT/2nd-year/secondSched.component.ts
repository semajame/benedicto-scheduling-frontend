...
export class secondSchedComponent implements AfterViewInit {
  ...
  generateAppointments(): void {
    ...
    forkJoin({
      minorSubjects: this.ccsService.findMinorSubjectsITSecondYear(),
      getSecondSchedules: this.ccsService.getSecondSchedules(),
    }).subscribe({
      next: ({ minorSubjects, getSecondSchedules }) => {
        // Clear previous conflicts
        this.conflicts = [];

        // Combine both sets of data into one array (you can change this logic based on your needs)
        const combinedData = [...minorSubjects, ...getSecondSchedules];
